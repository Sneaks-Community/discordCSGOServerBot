...
bot.on("ready", async () => {
  ...
  logChannel = bot.guilds.cache
    .get(config.logging.guildID)
    .channels.cache.get(config.logging.channelID);
...
async function addTrash(msg, om) {
  ...
  await msg.react("🗑️").then((reaction) => {
    const filter = (reaction, user) =>
      reaction.emoji.name === "🗑️" && user.id === om.author.id;
...
const command = args.shift().toLowerCase();
  ...
  if (command == "players" || command == "p") {
    ...
    // console.log(follows)
    if (follows.length == 0)
      return message.channel.send("You are not following any maps.");
    let list = "";
